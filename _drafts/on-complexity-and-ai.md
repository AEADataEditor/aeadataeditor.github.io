...
`Scons`, `snakemake`, `Makefile`, 

Original replicaiton package used `make`. I love me a nice `Makefile`, but... 50% of authors are on Windows, this fraction is even higher in "industry", and the solution "to run on Windows, install Linux"  is a bit facetious... and is not possible in many corporate environments. (I would like to hear back from folks on that, actually).

Authors proposed for Windows users to manually do the following

```ps1

   python code\download_dataverse.py --server="https://dataverse.harvard.edu" --persistent-id="doi:10.1234/DVN/XYZABC" --start="2010-01-01" --end="2027-12-31" 
   Rscript code\build_data.R --start=2010-01-01 --end=2027-12-31 --series=naics,enduse
   Rscript code\build_analysis.R
   Rscript code\build_table.R
```
...
```r
#!/usr/bin/env Rscript
CODE_DIR <- file.path("path", "to", "code")
DATA_DIR <- file.path("path", "to", "data")

# Step 1 still has to shell out — it's Python
system2(Sys.getenv("PYTHON", "python3"), c(
  file.path(CODE_DIR, "download_dataverse.py"),
  "--server=https://dataverse.harvard.edu",
  "--persistent-id=doi:10.7910/DVN/XYXABC",
  "--start=2010-01-01", "--end=2027-12-31",
  paste0("--imports-dir=", file.path(DATA_DIR, 'outputs', 'raw_zips')),
  paste0("--exports-dir=", file.path(DATA_DIR, 'outputs', 'raw_zips_exports')))
...
source_with_args(file.path(CODE_DIR, "build_analysis.R"))
source_with_args(file.path(CODE_DIR, "build_table.R"))
```
...
rootdir <- here::here()
CODE_DIR <- file.path(rootdir,"path","to","code")
DATA_DIR <- file.path(rootdir,"path","to","data")

source(file.path(rootdir,"config.R"), echo =TRUE)

# build args here
processx::run(pypath, args, echo = TRUE, error_on_status = TRUE)
source(file.path(CODE_DIR,"build_data.R"), echo =TRUE)
source(file.path(CODE_DIR,"build_analysis.R"), echo =TRUE)
source(file.path(CODE_DIR,"build_table.R"), echo =TRUE)
```
